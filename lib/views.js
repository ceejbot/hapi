...
internals.Manager.prototype.execute = function (engine, compiled, ctx, options) {
    ...
    } else if (engine.suffix === '.jazz') {
        return compiled.eval.bind(compiled);
    } else if (typeof compiled === 'function') {
        return compiled;
    }

    // we don't know how to execute this template type; should return an error
};
...
internals.Manager.prototype.render = function (filename, context, options, callback) {
    ...
    if (this.settings.asyncRender) {
        renderFunc = this.renderAsync.bind(this);
    } else {
        renderFunc = this.renderSync.bind(this);
    }

    if (typeof filename === 'string') {
        self._get(filename, options, function (err, template) {

            if (err) {
                return callback(err);
            }
            var engine = self.engines.get(self.engineLookup[filename]);
            renderFunc(engine, template, context, options, callback);
        });
    } else {
...
internals.Manager.prototype.renderAsync = function (engine, template, context, options, callback) {
    ...
    if (!self.settings.layout) {
        ...
        try {
            self.execute(engine, template, context, options)(context, function (err, result) {
                if (!result && (typeof err === 'string')) {
                    result = err;
                } else if (err) {
            ...
            callback(Boom.internal(err.message, err));
        }
        return;
    }
    ...
    if (context &&
        context.hasOwnProperty(self.settings.layoutKeyword)) {

        return Boom.internal('settings.layoutKeyword conflict', { context: context, keyword: self.settings.layoutKeyword });
    }

    self._get(this.settings.layoutFile, options, function (err, layout) {
        if (err) {
            return callback(err);
        }
        ...
        try {
            self.execute(engine, template, context, options)(context, function (err, tmplrendered) {
                if (err) {
                    return callback(Boom.internal(err.message, err));
                }

                layoutContext[self.settings.layoutKeyword] = tmplrendered;
                self.execute(engine, layout, layoutContext, options)(layoutContext, function(err, rendered) {
                    if (err) {
                        return callback(Boom.internal(err.message, err));
                    }
                    callback(rendered);
...
internals.Manager.prototype.renderSync = function (engine, template, context, options, callback) {
    if (!this.settings.layout) {

        // No layout
        var result;
        try {
            result = this.execute(engine, template, context, options)(context, options);
        }
        catch (err) {
            result = Boom.internal(err.message, err);
        }
    ...
    if (context &&
        ...
        return callback(Boom.internal('settings.layoutKeyword conflict', { context: context, keyword: this.settings.layoutKeyword }));
    }
    ...
    self._get(this.settings.layoutFile, options, function (err, layout) {
        ...
        var result;
        try {
            layoutContext[self.settings.layoutKeyword] = self.execute(engine, template, context, options)(context);
            result = self.execute(engine, layout, layoutContext, options)(layoutContext);
        }
        catch (err) {
            result = Boom.internal(err.message, err);
        }
...
internals.Manager.prototype._get = function (filename, options, callback) {

    options = this.mergeLeft(this.settings, options);

    if (this._isCaching(filename, options)) {
        return callback(null, this.getCache(filename));
    }
    ...
    if (this.settings.asyncCompile) {
        var self = this;

        engine.compile(source, options.compileOptions, function (err, compiled) {

            if (err) {
                return callback(err);
            }

            self.engineLookup[filename] = engine.suffix.slice(1);
            self.setCache(filename, compiled, options, self.settings.engines[engine.suffix.slice(1)]);
