...
internals.Manager.prototype.render = function (filename, context, options, callback) {
    ...
    if (typeof filename !== 'string') {
        return callback(Boom.internal('View manager was given a non-string template name', { filename: filename }));
    }
...
internals.Manager.prototype.renderAsync = function (engine, template, context, options, callback) {
    ...
    var _render = function (tmpl, ctx, cb) {
        try {
            self.execute(engine, tmpl, ctx, options)(ctx, function (err, result) {
                if (err) {
            ...
            cb(Boom.internal(err.message, err));
        }
    };

    // No layout
    if (!self.settings.layout) {
        return _render(template, context, callback);
    }

    // With layout

    if (context && context.hasOwnProperty(self.settings.layoutKeyword)) {
        return callback(Boom.internal('settings.layoutKeyword conflict',
            { context: context, keyword: self.settings.layoutKeyword }
        ));
    }
    ...
    self._get(this.settings.layoutFile, options, function (err, layout) {
        ...
        _render(template, context, function (content) {
            if (content instanceof Error) {
                return callback(content);
            }

            layoutContext[self.settings.layoutKeyword] = content;
            _render(layout, layoutContext, callback);
        });
    });
};
...
internals.Manager.prototype.renderSync = function (engine, template, context, options, callback) {
    ...
    var _render = function (tmpl, ctx) {
        var result;
        try {
            result = self.execute(engine, tmpl, ctx, options)(ctx, options);
        }
        catch (err) {
            result = Boom.internal(err.message, err);
        }
    ...
    if (!this.settings.layout) {
        return callback(_render(template, context));
    }

    // With layout
    if (context && context.hasOwnProperty(this.settings.layoutKeyword)) {
    ...
    self._get(this.settings.layoutFile, options, function (err, layout) {
        ...
        var layoutContext = Utils.clone(context);
        var content = _render(template, context);
        if (content instanceof Error) {
            return callback(content);
        }

        layoutContext[self.settings.layoutKeyword] = content;
        callback(_render(layout, layoutContext));
    });
};
